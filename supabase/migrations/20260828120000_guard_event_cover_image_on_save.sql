...
-- and two candidates would leave the call ambiguous, so retire the old one.
drop function if exists public.save_admin_event_atomically(
  uuid,
  jsonb,
  text,
  jsonb,
...
begin
  if p_event_id is null then
    ...
    v_next_image_url := p_event ->> 'image_url';
    v_image_changed := v_next_image_url is distinct from p_expected_image_url;
    ...
    set
      ...
      -- A save that leaves the cover alone leaves the column alone too:
      -- writing the loaded value back would revert a replacement made
      -- elsewhere to a URL whose object that replacement already deleted.
      image_url = case
        when v_image_changed then v_next_image_url
        else image_url
      end,
      registration_start_time =
        (p_event ->> 'registration_start_time')::timestamptz,
      registration_end_time =
        (p_event ->> 'registration_end_time')::timestamptz
    where id = p_event_id
      -- Changing the cover deletes the old object, so that write is
      -- conditional on the row still holding the image this form loaded with.
      and (
        not v_image_changed
        or image_url is not distinct from p_expected_image_url
      )
    returning id into v_event_id;

    if v_event_id is null then
      if exists (select 1 from public.events where id = p_event_id) then
        -- Matched by the caller, which turns it into a reload prompt.
        raise exception 'EVENT_IMAGE_CONFLICT';
      end if;
